const LeafSpineCalculator = (() => {
  ...
  function buildInfeasibleReason(input, totalServerLinks, stats) {
    ...
    if (stats.leafServerPortShortage > 0) {
      ...
      reasons.push(`Leaf 서버 다운링크 포트 부족: 서버 연결 링크 ${totalServerLinks.toLocaleString()}개를 Leaf에 분산해도 Leaf당 최소 ${requiredDownlinkPorts.toLocaleString()}개의 물리 포트가 필요합니다. 현재 스위치는 ${input.switchPorts.toLocaleString()}포트입니다.`);
    }
    if (stats.leafTotalPortShortage > 0) {
      ...
      reasons.push(`Leaf 총 포트 부족: Leaf당 서버 다운링크와 Spine 업링크를 합산하면 최소 ${requiredLeafPorts.toLocaleString()}개의 물리 포트가 필요합니다. 현재 스위치는 ${input.switchPorts.toLocaleString()}포트입니다.`);
    }
    if (stats.spinePortShortage > 0 || stats.fullMeshShortage > 0) {
      ...
      const fullMeshDetail = stats.minRequiredSpinesForFullMesh > 0
        ...
        : "";
      reasons.push(`Spine 포트 또는 full-mesh 조건 부족: ${spineDetail}${fullMeshDetail}현재 스위치는 Spine당 ${input.switchPorts.toLocaleString()}포트를 사용할 수 있습니다.`);
    }
